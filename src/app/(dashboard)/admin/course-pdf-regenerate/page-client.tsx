...
export default function BatchCoursePdfRegeneratePage() {
  ...
  const [sortField, setSortField] = useState<"title" | "file_size" | null>(null);
  ...
  const fetchCurrentJob = useCallback(async () => {
    try {
      const res = await fetch("/api/batch/courses/current");
  ...
  // Fetch already processed course IDs (for resume)
  const fetchAlreadyProcessedIds = useCallback(async (jobId: string) => {
    try {
      ...
      const successRes = await fetch(`/api/batch/courses/${jobId}?${params}`);
      const successData = await successRes.json();
      if (successData.results) {
        successData.results.forEach((r: BatchResult) => {
          processedRef.current.add(r.course_id);
        });
      }

      const failedParams = new URLSearchParams({ pageSize: "1000", status: "failed" });
      const failedRes = await fetch(`/api/batch/courses/${jobId}?${failedParams}`);
  ...
  // Fetch pending courses for resume
  const fetchPendingCourses = useCallback(async (jobId: string): Promise<string[]> => {
    try {
      const res = await fetch(`/api/batch/courses/${jobId}/pending`);
      const data = await res.json();
      if (data.pendingCourses) {
        return data.pendingCourses.map((p: any) => p.course_id);
      }
      return [];
    } catch (error) {
  ...
  const fetchResults = useCallback(async (page: number = 1, filter: "all" | "success" | "failed" = "all") => {
    ...
    try {
      ...
      const res = await fetch(`/api/batch/courses/${currentJobId}?${params}`);
      if (!res.ok) {
  ...
  // Process a single course PDF
  const processCourse = async (courseId: string, jobId: string): Promise<{ success: boolean; error?: string }> => {
    try {
      // First attempt
      const res = await fetch(`/api/courses/${courseId}/pdf/generate`, { method: "POST" });
      const data = await res.json();
      ...
      // Retry once
      const retryRes = await fetch(`/api/courses/${courseId}/pdf/generate`, { method: "POST" });
  ...
  const submitResult = async (jobId: string, courseId: string, success: boolean, errorMsg?: string) => {
    try {
      await fetch(`/api/batch/courses/${jobId}/results`, {
        method: "POST",
  ...
  const getSortedResults = () => {
    ...
    return [...results].sort((a, b) => {
      ...
      if (sortField === "title") {
        ...
        aVal = a.course?.file_size ?? 0;
        bVal = b.course?.file_size ?? 0;
      }
  ...
  // Retry failed courses (reset them to pending)
  const retryFailed = async () => {
    if (!jobIdRef.current) return;

    if (!confirm("This will reset all failed courses to pending. Continue?")) return;

    setLoading(true);
    try {
      const res = await fetch(`/api/batch/courses/${jobIdRef.current}/retry-failed`, {
        method: "POST",
      });
      const data = await res.json();

      if (!res.ok) {
        alert(data.error || "Failed to retry failed courses");
      } else {
        console.log("[Retry] Reset", data.count, "failed courses to pending");
  ...
  const resumeBatch = async () => {
    ...
    try {
      ...
      const pendingCourseIds = await fetchPendingCourses(job.id);
      console.log("[Resume] Pending courses:", pendingCourseIds.length);
      processingQueueRef.current = pendingCourseIds;

      setIsRunning(true);

      // Process pending courses
      for (const courseId of processingQueueRef.current) {
        if (!processingRef.current) {
          console.log("[Resume] Processing cancelled by user");
          break;
        }
        if (processedRef.current.has(courseId)) {
          console.log("[Resume] Already processed:", courseId);
          continue;
        }

        console.log("[Resume] Processing course:", courseId);
        const result = await processCourse(courseId, job.id);
        console.log("[Resume] Course result:", courseId, result.success ? "SUCCESS" : "FAILED", result.error);
        await submitResult(job.id, courseId, result.success, result.error);
        processedRef.current.add(courseId);
  ...
  const startBatch = async () => {
    ...
    try {
      ...
      // Start the batch job
      const res = await fetch("/api/batch/course-pdf-regenerate", { method: "POST" });
      const data = await res.json();
      ...
      if (!coursesRes.ok) {
        console.error("[Batch] Failed to fetch courses, status:", coursesRes.status);
        alert("Failed to fetch courses");
        setLoading(false);
        return;
      }
      const coursesData = await coursesRes.json();
      console.log("[Batch] Courses response:", coursesData);
      const courseIds = (coursesData.courses as any[])?.map((c) => c.id as string) || [];
      console.log("[Batch] Course IDs count:", courseIds.length);
      processingQueueRef.current = [...new Set(courseIds)] as string[]; // Deduplicate
      ...
      for (const courseId of processingQueueRef.current) {
        ...
        if (processedRef.current.has(courseId)) {
          console.log("[Batch] Already processed:", courseId);
          continue;
        }

        console.log("[Batch] Processing course:", courseId);
        const result = await processCourse(courseId, jobId);
        console.log("[Batch] Course result:", courseId, result.success ? "SUCCESS" : "FAILED", result.error);
        await submitResult(jobId, courseId, result.success, result.error);
        processedRef.current.add(courseId);
  ...
  // View log for a failed course
  const viewLog = async (courseId: string, errorMessage: string | null = null) => {
    ...
    try {
      // For course PDFs, we don't have a diagnostics endpoint like lessons do
      // So we just show the error message
      setLogData({ error: errorMessage || "No additional diagnostics available" });
    } catch (err) {
    ...
    <div className="min-h-screen bg-white">
      ...
      {job?.status === "processing" && (
        <div className="bg-[#0d7377] text-white px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="font-medium">Batch Course PDF Regeneration in Progress</span>
            ...
            <button
              onClick={async () => {
                if (confirm("Are you sure you want to cancel the batch?")) {
                  ...
                  await fetch(`/api/batch/courses/${job.id}/cancel`, { method: "POST" });
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold text-[#2d2d2d]">Batch Course PDF Regeneration</h1>
            ...
            <div className="flex items-center gap-3">
              ...
              <Button
                onClick={() => {
                  if (confirm("Are you sure you want to cancel this batch?")) {
                    fetch(`/api/batch/courses/${job.id}/cancel`, { method: "POST" });
                    fetchCurrentJob();
                  }
                }}
                variant="outline"
              ...
              <Button
                onClick={async () => {
                  ...
                  const res = await fetch(`/api/batch/courses/${job.id}/sync-counters`, { method: "POST" });
                  const data = await res.json();
                  if (data.success) {
              ...
              <tbody className="divide-y divide-gray-200">
                {getSortedResults().map((result) => (
                  <tr key={result.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-900">
                      {formatFileSize(result.course?.file_size ?? null)}
                    </td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex justify-end gap-2">
                        {result.status === "failed" && (
                          <button
                            onClick={() => viewLog(result.course_id, result.error_message)}
                        ...
                        <button
                          onClick={() => window.open(`/admin/courses/${result.course_id}`, "_blank")}
                          className="p-1 hover:bg-gray-200 rounded text-gray-600"
                          title="Edit Course"
                        >
                        ...
                        <button
                          onClick={() => window.open(`/api/courses/${result.course_id}/pdf?download=false`, "_blank")}
                          className="p-1 hover:bg-gray-200 rounded text-gray-600"
                          title="View PDF"
                        >
        ...
        {!job && !loading && (
          <div className="text-center py-12 text-gray-500">
            <p>No batch jobs yet. Click "Start New Batch" to begin regenerating all course PDFs.</p>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            ...
            <p className="text-gray-600 mb-6">
              {confirmMode === "resume" ? (
                <>This will resume processing <strong>{job?.total_count && job?.processed_count ? job.total_count - job.processed_count : "the remaining"}</strong> pending courses from where the batch stalled. Previous results are preserved.</>
              ) : (
                <>This will create a NEW batch job and regenerate PDFs for all courses. The previous batch will remain in history for reference.</>
              )}
            </p>
          ...
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
            <div className="flex items-center justify-between p-4 border-b">
              <h2 className="text-xl font-bold text-[#2d2d2d]">
                PDF Diagnostics {selectedCourseId && `- Course ${selectedCourseId.slice(0, 8)}...`}
              </h2>
