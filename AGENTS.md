...
- **Filters + pagination**: server-side via `GET /api/admin/teachers?status=&role=&discipline=…&search=&limit=&offset=`. Toolbar (search + status + role + multi-select discipline) sits right of the search input; URL is the single source of truth (`useSearchParams` + `router.replace` with `scroll: false`). Page size is 50; ordering is `created_at DESC, id ASC` for stable page boundaries. Discipline uses a chip-popover multi-select (`src/components/multi-select-popover.tsx`) since it's the only multi-value filter. Selection state survives pagination; bulk save/delete refreshes the current page only.
...
- **Backfill (migration 031)**: all existing active teachers with `access_ends_at IS NULL` are set to `2027-12-31` at noon UTC. The migration also includes a corrective `UPDATE` shifting existing rows from `00:00:00Z` to `12:00:00Z` for the same reason.
- **Noon-UTC storage convention**: `access_ends_at` and `trial_ends_at` are stored at **noon UTC** on the target calendar date. Storing at noon keeps the same calendar day in every timezone (noon is the same day everywhere on Earth), so the rendered date matches the date the admin actually picked. **Storing at midnight UTC is wrong** — it renders as the previous calendar day anywhere west of UTC. Use `localDateInputToUtcNoon('YYYY-MM-DD')` from `src/lib/access-utils.ts` for client-to-server writes; use `utcNoonInDaysFromNow(n)` for `now + n days` (e.g., 14-day trial). Display helpers `isoToLocalDateInput` / `isoToLocalDateDisplay` use the local calendar date for rendering.
...
4. **get_course_lessons**: Get all lessons in a course with full content
   ...
   - Returns all lessons for the course with full content (htmlToPlainText applied)
...
**Course Scope Special Handling**:
- `search_lessons` is **DEACTIVATED** for Course scope
