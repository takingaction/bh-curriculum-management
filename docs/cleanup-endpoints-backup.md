# Cleanup Endpoints Backup

These endpoints were consolidated into `/api/cleanup-html` on May 29, 2026.

---

## cleanup-nested-tags (DELETED)
...
## cleanup-anchor-standard (DELETED)
      ...
      for (const field of contentFields) {
        if (lesson[field]) {
          ...
          const transformed = original
            ...
            .replace(/<h3((?:PK\.MU|PK|MU)\:[^<]*)<\/h3>/gi, '<p><strong$1</strong></p>');
...
## Consolidated into: cleanup-html

See `/src/app/api/cleanup-html/route.ts` for the current implementation which combines all cleanup logic.
